...
void create_Transpiler_context_t(Transpiler_context_t* ctx, const char* filepath) {
    ...
    if (ctx->filepath == NULL) memory_allocation_failure(__FILE__, __LINE__);
    ...
    if (ctx->tokens == NULL) memory_allocation_failure(__FILE__, __LINE__);

    // Initialize parser
    ctx->nodes_count = 0;
    ctx->nodes_capacity = 8;
    ctx->nodes =  calloc(ctx->nodes_capacity, sizeof(Node_t));
    if (ctx->nodes == NULL) memory_allocation_failure(__FILE__, __LINE__);
...
size_t add_to_indices(size_t* indices_list, size_t index, size_t count, size_t capacity) {
    if (count >= capacity) {
        ...
        if (indices_list == NULL) memory_allocation_failure(__FILE__, __LINE__);
    }
    size_t at = count;
